...
public class GetBillersPostProcessor implements DataPostProcessor {

 private static final Logger LOG = Logger
  .getLogger(GetBillersPostProcessor.class.getName());
 @Override
  ...
  try {
   if (results != null) {
    ...
    if (ds.size() == 0) {
     ...
     newDataSet.setId("BillerCompany");
     ...
     for (Dataset dataset: ds) {
      ...
      if(records.size()==0)
      {
    	  LOG.debug("Inside record is zero:");
    	     Dataset newDataSet = new Dataset();
    	     newDataSet.setId("BillerCompany");
    	     Record record = new Record();
    	     if (statusCode != null && !"".equals(statusCode.toString())) {
