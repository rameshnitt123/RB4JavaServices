...
public class AccountsPostProcessor implements DataPostProcessor {

 private static final Logger LOG = Logger
  .getLogger(AccountsPostProcessor.class.getName());
 @Override
 public Object execute(Result results, DataControllerRequest request) throws Exception {
  try {
   if (results != null) {
	   ...
	   Param nextAvailableScheduledTransferDate = results.findParam("nextAvailableScheduledTransferDate");
    ...
    if (ds.size() == 0) {
     ...
     if (nextAvailableScheduledTransferDate != null && !"".equals(nextAvailableScheduledTransferDate.toString())) {
    	 record.setParam(nextAvailableScheduledTransferDate);
 	 }
      ...
      if (records.size() == 0) {
       ...
       for (Record record: records) {
        if (records.indexOf(record) == 0) {
         ...
         if (nextAvailableScheduledTransferDate != null && !"".equals(nextAvailableScheduledTransferDate.toString())) {
        	 params.add(nextAvailableScheduledTransferDate);
     	 }
         if (lastUpdateTimeStamp != null && !"".equals(lastUpdateTimeStamp.toString())) {
